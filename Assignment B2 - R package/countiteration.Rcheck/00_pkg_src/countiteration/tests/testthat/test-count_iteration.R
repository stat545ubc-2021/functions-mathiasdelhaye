test_that("counting_iteration function", {
  expect_equal(
    as.character(rlang::get_expr(counting_iteration(datateachr::vancouver_trees, neighbourhood_name)$mapping$y)), "n") #checking the name of the y axis
  expect_equal(
    as.character(rlang::get_expr(counting_iteration(datateachr::vancouver_trees, neighbourhood_name)$mapping$x)), "neighbourhood_name"
